...
import { Sparkles, Crosshair, Target, Swords, Trophy, Diamond, Flame, Snail, Siren, Spade as FootballIcon, Waves as RugbyIcon, Badge as VolleyballIcon, Wind } from "lucide-react";
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
        ...
        {SPORT_CARDS.map((sport, i) => {
          ...
          return (
            ...
            >
              {/* Gradient background */}
              <div
                className={`absolute inset-0 bg-gradient-to-br from-emerald-500/10 to-transparent opacity-30`}
              />
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <div className="mb-4"><Sparkles className="h-12 w-12 text-zinc-700" /></div>
          <h3 className="text-sm font-medium text-zinc-400">
            {activeSport === "all"
              ? "No matches found across any sport"                    : `No ${SPORT_CONFIG[activeSport]?.label ?? activeSport} matches right now`}
          </h3>
          <p className="mt-2 text-xs text-zinc-600">
            Matches will appear here once data is available from the sports APIs.
            Check back during live game times.
          </p>
        </div>
      ) : (
