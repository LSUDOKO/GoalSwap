...
const defaultBrackets: BracketInfo[] = [
  {
    ...
    minted: 142,
    total: 500,
    description: "Predict all 8 Round of 16 winners correctly. The foundation of your bracket run.",
    icon: "R16",
  ...
  },
  {
    ...
    minted: 89,
    total: 500,
    description: "4 crucial matches — predict every QF winner to advance. Where brackets start to separate.",
    icon: "QF",
  ...
  },
  {
    ...
    minted: 67,
    total: 500,
    description: "The final four. Predict both semi-final winners correctly to earn elite status.",
    icon: "SF",
  ...
  },
  {
    ...
    minted: 203,
    total: 1000,
    description: "The championship match — predict the World Cup winner. The ultimate test of foresight.",
    icon: "FIN",
...
export default function BracketsPage() {
  ...
  const [brackets, setBrackets] = useState<BracketInfo[]>(defaultBrackets);
  ...
  const handleMint = async (round: string) => {
    setMintingId(round);
    // Simulate minting
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      >
        {[
          { label: "Total Minted", value: "501", icon: Users },
          { label: "Your Brackets", value: isConnected ? "2" : "—", icon: Shield },
          ...
          return (
            ...
            >
              ...
              <div className="text-sm font-bold text-zinc-100 tabular-nums">{stat.value}</div>
      ...
      {loading ? (
        <div className="grid gap-6 sm:grid-cols-2">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="animate-pulse rounded-xl border border-zinc-800 bg-zinc-900/50 p-6">
              ...
              <div className="h-3 w-3/4 rounded bg-zinc-800 mb-4" />
              ...
              >
                ...
                <div className="relative">
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-sm font-semibold text-zinc-100">{bracket.round}</h3>
                      <p className="text-[10px] text-zinc-500">{bracket.matches} match{bracket.matches !== 1 ? "es" : ""}</p>
                    </div>
                  ...
                  {/* Description */}
                  <p className="text-xs text-zinc-500 leading-relaxed mb-4">{bracket.description}</p>

                  {/* Progress */}
                  <div className="mb-4">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-[10px] text-zinc-600">Minted</span>
                    ...
                    <div className="h-2 rounded-full bg-zinc-800 overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: `${Math.min((bracket.minted / bracket.total) * 100, 100)}%` }}
                  ...
                  <div className="mb-4 rounded-lg bg-zinc-800/30 p-2.5">
                    <div className="flex items-center justify-between text-[10px]">
                      <span className="text-zinc-500">Reward on correct prediction</span>
                      <span className="text-emerald-400 font-medium">{bracket.reward}</span>
                    </div>
                  </div>
                  ...
                  {isConnected ? (
                    <button
                      onClick={() => handleMint(bracket.round)}
                      disabled={mintingId === bracket.round}
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-sm font-semibold text-zinc-100">How Bracket NFTs Work</h2>
        </div>
        <div className="grid gap-4 sm:grid-cols-3 text-xs text-zinc-500 leading-relaxed">
          <div className="rounded-lg bg-zinc-800/30 p-3">
            <span className="text-emerald-400 font-semibold block mb-1">1. Mint & Predict</span>
            Mint a bracket NFT with your World Cup predictions. Each round is a separate NFT market — mint the rounds you're confident in.
          </div>
          <div className="rounded-lg bg-zinc-800/30 p-3">
            <span className="text-emerald-400 font-semibold block mb-1">2. Trade on Secondary</span>
            Brackets are transferable ERC-721 tokens. Trade them on secondary markets before the tournament ends — early predictions gain value.
          </div>
          <div className="rounded-lg bg-zinc-800/30 p-3">
            <span className="text-emerald-400 font-semibold block mb-1">3. Earn Rewards</span>
            Correct predictions earn XP and soulbound trophies. The more rounds you predict correctly, the higher your leaderboard rank.
          </div>
        </div>
      </motion.div>
    </div>
